@extends('layouts.app')
@section('title', 'Modifier — '.$film->title)
...
<div class="max-w-3xl">
    <form method="POST" action="{{ route('admin.films.update', $film) }}" enctype="multipart/form-data" class="space-y-6">
        @csrf @method('PUT')
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <div class="sm:col-span-2">
                    ...
                    <input type="text" name="original_title" value="{{ old('original_title', $film->original_title) }}"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                </div>
                <div class="sm:col-span-2">
                    <label class="block text-xs font-medium text-gray-600 mb-1">Description</label>
                    <textarea name="description" rows="4"
                              class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none">{{ old('description', $film->description) }}</textarea>
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-600 mb-1">Genre</label>
                    <input type="text" name="genre" value="{{ old('genre', $film->genre) }}" placeholder="Drame, Action, Comédie…"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-600 mb-1">Année de sortie</label>
                    <input type="number" name="release_year" value="{{ old('release_year', $film->release_year) }}" min="1900" max="2100"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-600 mb-1">Durée (minutes)</label>
                    <input type="number" name="duration" value="{{ old('duration', $film->duration) }}" min="1" max="999"
                           class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                </div>
                <div>
                    ...
                    <div class="flex items-center gap-2 mt-2">
                        <input type="checkbox" name="is_active" id="is_active" value="1"
                               {{ old('is_active', $film->is_active) ? 'checked' : '' }}
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <div class="space-y-4">
                <div>
                    <label class="block text-xs font-medium text-gray-600 mb-1">Affiche actuelle</label>
                    @if($film->poster_path)
                    <img src="{{ Storage::url($film->poster_path) }}" alt="{{ $film->title }}"
                         class="h-32 rounded-lg object-cover shadow-sm mb-2">
                    @else
                    <p class="text-xs text-gray-400 mb-2">Aucune affiche</p>
                    @endif
                    ...
                    <p class="text-xs text-gray-400 mt-1">Laisser vide pour conserver l'affiche actuelle.</p>
                ...
                <div>
                    ...
                    <input type="text" name="hls_manifest_url" value="{{ old('hls_manifest_url', $film->hls_manifest_url) }}"
                ...
                <div>
                    ...
                    <input type="text" name="drm_key" value="{{ old('drm_key', $film->drm_key) }}"
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" name="price" value="{{ old('price', $film->price) }}" min="0" step="1" required
                ...
                <div>
                    ...
                    <select name="currency" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                        ...
                        <option value="{{ $code }}" {{ old('currency', $film->currency) === $code ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
