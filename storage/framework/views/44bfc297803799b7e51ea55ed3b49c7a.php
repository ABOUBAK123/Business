<?php $__env->startSection('title', 'Gestion Courrier'); ?>
<?php $__env->startSection('page-title', 'Gestion Courrier'); ?>
<?php $__env->startSection('page-subtitle', 'Enregistrement, suivi et traitement des courriers'); ?>
<?php $__env->startSection('content'); ?>


<div class="flex flex-wrap gap-1.5 mb-6 bg-white rounded-2xl border border-gray-200 p-2 shadow-sm">
    <?php $__currentLoopData = $subtabs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $meta): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
    <a href="<?php echo e(route('courrier.' . $key)); ?>"
       ...
       class="flex items-center gap-1.5 px-3 py-2 sm:px-4 sm:py-2.5 rounded-xl text-xs sm:text-sm font-semibold transition
              <?php echo e($subtab === $key ? 'bg-[#2453d6] text-white shadow-sm' : 'text-gray-600 hover:bg-gray-100'); ?>">
        <i class="<?php echo e($meta['icon']); ?> text-xs flex-shrink-0"></i>
        <span class="hidden sm:inline"><?php echo e($meta['label']); ?></span>
    </a>
    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
</div>


<?php if($subtab === 'enregistrement'): ?>
<?php $typeForm = request('type_courrier', 'arrive'); ?>
<?php $prochainNumero = $prochainNumero ?? (($typeForm === 'arrive' ? 'A' : 'D') . '-0001-DIR001-' . date('Y')); ?>
    ...
    <div class="grid grid-cols-2 mb-6 bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        <a href="?type_courrier=arrive"
           class="text-center py-4 text-sm font-semibold transition border-b-2
                  <?php echo e($typeForm === 'arrive' ? 'border-blue-600 text-blue-600 bg-blue-50/40' : 'border-transparent text-gray-400 hover:text-gray-600 hover:bg-gray-50'); ?>">
            Courrier Arrivé
        </a>
        <a href="?type_courrier=depart"
           class="text-center py-4 text-sm font-semibold transition border-b-2
                  <?php echo e($typeForm === 'depart' ? 'border-blue-600 text-blue-600 bg-blue-50/40' : 'border-transparent text-gray-400 hover:text-gray-600 hover:bg-gray-50'); ?>">
            Courrier Départ
        </a>
    </div>

    <form method="POST" action="<?php echo e(route('courrier.store')); ?>" enctype="multipart/form-data" class="space-y-5">
        <?php echo csrf_field(); ?>
        <input type="hidden" name="type_courrier" value="<?php echo e($typeForm); ?>">
        ...
        <div class="flex items-center justify-between bg-indigo-50 border border-indigo-200 rounded-xl px-4 py-3">
            <div class="flex items-center gap-2 text-sm text-indigo-700 font-medium">
                ...
                Prochain numéro : <span class="font-bold"><?php echo e($prochainNumero); ?></span>
            </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-5 space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label class="block text-xs font-semibold text-gray-700 mb-1">
                        <?php echo e($typeForm === 'arrive' ? 'Expéditeur' : 'Destinataire'); ?> <span class="text-red-500">*</span>
                    </label>
                    <textarea name="<?php echo e($typeForm === 'arrive' ? 'expediteur' : 'destinataire'); ?>" rows="3" required
            ...
            <?php if($typeForm === 'depart'): ?><?php endif; ?>

            <div>
                <label class="block text-xs font-semibold text-gray-700 mb-1">
                    <?php echo e($typeForm === 'depart' ? "Accusé de réception" : 'Observations'); ?>

                </label>
                <?php if($typeForm === 'depart'): ?>
...
function startOcr(file) {
    ...
    fetch('<?php echo e(route("courrier.scan-ocr")); ?>', {
            method: 'POST',
...
<div class="flex flex-wrap items-center justify-between gap-3 mb-5">
    ...
    ?>
    <?php if($canCreateCourrier): ?>
    <a href="<?php echo e(route('courrier.enregistrement')); ?>"
...
<form method="GET" action="<?php echo e(route('courrier.liste')); ?>" class="mb-5">
    <div class="flex flex-wrap gap-3 items-center">
        
        <div class="flex gap-2">
            <?php $__currentLoopData = ['tous'=>'Tous','arrive'=>'Arrivés','depart'=>'Départs']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $val=>$label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <button type="submit" name="filtre" value="<?php echo e($val); ?>"
                class="<?php echo e(($filtre??'tous')===$val ? 'bg-blue-600 text-white shadow-sm' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50'); ?> px-5 py-2 rounded-xl text-sm font-semibold transition">
                <?php echo e($label); ?>

                <?php if($val !== 'tous' && isset($statut)): ?> <input type="hidden" name="statut" value="<?php echo e($statut); ?>"><?php endif; ?>
            </button>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </div>
        ...
        <select name="statut" onchange="this.form.submit()"
            ...
            <option value="" <?php echo e(($statut??'')=='' ? 'selected' : ''); ?>>Tous les statuts</option>
            <option value="En attente"    <?php echo e(($statut??'')==='En attente'    ? 'selected' : ''); ?>>En attente</option>
            <option value="En traitement" <?php echo e(($statut??'')==='En traitement' ? 'selected' : ''); ?>>En traitement</option>
            <option value="Traité"        <?php echo e(($statut??'')==='Traité'        ? 'selected' : ''); ?>>Traité</option>
        </select>
        ...
        <?php if(($filtre??'tous') !== 'tous' || ($statut??'') !== '' || ($search??'') !== ''): ?>
        <a href="<?php echo e(route('courrier.liste')); ?>"
...
<p class="text-xs text-gray-400 mb-3"><?php echo e(count($courriers)); ?> courrier(s) trouvé(s)</p>
...
<div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
    <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-gray-100">
            <?php $__empty_1 = true; $__currentLoopData = $courriers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $c): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            <tr class="hover:bg-gray-50 transition">
                <td class="px-6 py-4 font-bold text-gray-900 whitespace-nowrap"><?php echo e($c['num']); ?></td>
                <td class="px-6 py-4 text-gray-700"><?php echo e($c['objet']); ?></td>
                <td class="px-6 py-4">
                    <span class="inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-1 rounded-full <?php echo e($c['type']==='Arrivé' ? 'bg-indigo-50 text-indigo-700' : 'bg-orange-50 text-orange-700'); ?>">
                        <i class="fas <?php echo e($c['type']==='Arrivé' ? 'fa-arrow-down' : 'fa-arrow-up'); ?> text-[10px]"></i>
                        <?php echo e($c['type']); ?>

                    </span>
                </td>
                <td class="px-6 py-4">
                    <?php $priClass = match($c['priorite']) { 'Urgent'=>'text-orange-600 font-bold', 'Très urgent'=>'text-red-600 font-bold', default=>'text-blue-600 font-semibold' }; ?>
                    <span class="<?php echo e($priClass); ?> text-sm"><?php echo e($c['priorite']); ?></span>
                </td>
                <td class="px-6 py-4">
                    ...
                    <span class="<?php echo e($badgeClass); ?> px-3 py-1 rounded-full text-xs font-semibold"><?php echo e($c['statut']); ?></span>
                </td>
                <td class="px-6 py-4 text-gray-700"><?php echo e($c['agent']); ?></td>
                <td class="px-6 py-4 text-right">
                    <div class="flex items-center justify-end gap-2">
                        <button onclick="openPdfModal('<?php echo e($c['fichier'] ?? ''); ?>', '<?php echo e(addslashes($c['num'])); ?>')"
                        ...
                        <?php if(($c['statut'] ?? '') !== 'Traité'): ?>
                        <button onclick="openEditModal('<?php echo e($c['num']); ?>','<?php echo e(addslashes($c['objet'])); ?>','<?php echo e($c['type']); ?>','<?php echo e($c['priorite']); ?>','','','<?php echo e(addslashes($c['expediteur'] ?? '')); ?>')"
...
<?php elseif($subtab === 'imputation'): ?>


<?php if(!empty($accesDenied)): ?>
...
<div class="flex flex-wrap items-center justify-between gap-3 mb-5">
    ...
    <span class="text-xs bg-blue-50 text-blue-700 border border-blue-100 px-3 py-1.5 rounded-full font-semibold">
        <?php echo e($courriers->count()); ?> courrier(s) à imputer
    </span>
</div>


<form method="GET" action="<?php echo e(route('courrier.imputation')); ?>" class="flex flex-wrap gap-3 items-center mb-5">
    <div class="flex gap-2">
        <?php $__currentLoopData = ['tous'=>'Tous','urgent'=>'Urgents']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $val=>$label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <button type="submit" name="imp_filtre" value="<?php echo e($val); ?>"
            class="<?php echo e(($imputFiltre??'tous')===$val ? 'bg-blue-600 text-white shadow-sm' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50'); ?> px-5 py-2 rounded-xl text-sm font-semibold transition">
            <?php echo e($label); ?>
    ...
    <div class="flex-1 min-w-[200px] relative">
        <input type="text" name="imp_q" value="<?php echo e($imputSearch ?? ''); ?>"
    ...
    <?php if(($imputFiltre??'tous')!=='tous' || ($imputSearch??'')!==''): ?>
    <a href="<?php echo e(route('courrier.imputation')); ?>" class="px-4 py-2 bg-gray-100 text-gray-600 rounded-xl text-sm font-semibold hover:bg-gray-200 transition flex items-center gap-1.5">
        <i class="fas fa-times text-xs"></i> Réinitialiser
    </a>
    <?php endif; ?>
</form>
...
<div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
    <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-gray-100">
            <?php $__empty_1 = true; $__currentLoopData = $courriers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $c): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            <tr class="hover:bg-gray-50 transition">
                <td class="px-6 py-5 font-bold text-gray-900 whitespace-nowrap"><?php echo e($c->numero); ?></td>
                <td class="px-6 py-5 text-gray-700"><?php echo e($c->objet); ?></td>
                <td class="px-6 py-5 text-gray-600"><?php echo e($c->expediteur ?? '—'); ?></td>
                <td class="px-6 py-5">
                    <?php
                    ...
                    ?>
                    <span class="<?php echo e($priColor); ?>"><?php echo e($c->priorite_libelle); ?></span>
                </td>
                <td class="px-6 py-5 font-semibold text-gray-700 uppercase">
                    <?php echo e($c->enregistrePar?->full_name ?? $c->enregistrePar?->name ?? '—'); ?>

                </td>
                <td class="px-6 py-5 text-right">
                    <div class="flex items-center justify-end gap-2">
                        <button onclick="openPdfModal(
                                '<?php echo e($c->pieces_jointes && count($c->pieces_jointes) ? asset('storage/'.$c->pieces_jointes[0]) : ($c->accuse_reception ? asset('storage/'.$c->accuse_reception) : '')); ?>',
                                '<?php echo e(addslashes($c->numero)); ?>')"
                        ...
                        <button onclick="openImputerModal('<?php echo e($c->id); ?>','<?php echo e($c->numero); ?>','<?php echo e(addslashes($c->objet)); ?>','<?php echo e(addslashes($c->expediteur ?? '')); ?>')"
...
<?php elseif($subtab === 'en-traitement'): ?>
<?php
...
<div class="flex flex-wrap items-center justify-between gap-3 mb-5">
    ...
    <span class="text-xs bg-orange-50 text-orange-700 border border-orange-100 px-3 py-1.5 rounded-full font-semibold">
        <?php echo e($rows->count()); ?> courrier(s) en traitement
    </span>
</div>


<form method="GET" action="<?php echo e(route('courrier.en-traitement')); ?>" class="flex flex-wrap gap-3 items-center mb-5">
    <div class="flex-1 min-w-[200px] relative">
        <input type="text" name="et_q" value="<?php echo e($etSearch ?? ''); ?>"
    ...
    <?php if(($etSearch ?? '') !== ''): ?>
    <a href="<?php echo e(route('courrier.en-traitement')); ?>" class="px-4 py-2 bg-gray-100 text-gray-600 rounded-xl text-sm font-semibold hover:bg-gray-200 transition flex items-center gap-1.5">
        <i class="fas fa-times text-xs"></i> Réinitialiser
    </a>
    <?php endif; ?>
</form>
...
<div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
    <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-gray-100">
            ...
            <tr class="hover:bg-gray-50 transition">
                <td class="px-6 py-5 font-bold text-gray-900 whitespace-nowrap"><?php echo e($c['num']); ?></td>
                <td class="px-6 py-5 text-gray-700"><?php echo e($c['objet']); ?></td>
                <td class="px-6 py-5 text-gray-600"><?php echo e($c['expediteur'] ?: '—'); ?></td>
                <td class="px-6 py-5 font-semibold text-gray-700 uppercase"><?php echo e($c['imputer_par_code']); ?></td>
                <td class="px-6 py-5 text-gray-600"><?php echo e($c['delai']); ?></td>
                <td class="px-6 py-5 text-right">
                    <div class="flex items-center justify-end gap-2">
                        <button onclick="openPdfModal('<?php echo e($c['fichier'] ?? ''); ?>', '<?php echo e(addslashes($c['num'])); ?>')"
                            class="inline-flex items-center justify-center w-9 h-9 rounded-full bg-blue-600 hover:bg-blue-700 text-white transition shadow-sm" title="Voir le courrier">
                        ...
                        <button onclick="openTraiterModal('<?php echo e($c['id']); ?>','<?php echo e($c['num']); ?>','<?php echo e(addslashes($c['objet'])); ?>','<?php echo e(addslashes($c['expediteur'] ?? '')); ?>','<?php echo e($c['delai']); ?>')"
...
<div id="modal-traiter" class="fixed inset-0 z-50 hidden flex items-center justify-center p-4">
    ...
    <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg">
        ...
        <form method="POST" action="<?php echo e(route('courrier.traiter')); ?>" enctype="multipart/form-data" class="p-5 space-y-4">
            <?php echo csrf_field(); ?>
...
<?php elseif($subtab === 'suivi-imputation'): ?>
<?php
...
<div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
    ...
    <div class="px-6 py-5 border-b border-gray-100 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
            ...
            <p class="text-xs text-gray-400 mt-1"><?php echo e($suiviImputations->count()); ?> courrier(s) trouvé(s)</p>
        </div>
        <form method="GET" action="<?php echo e(route('courrier.suivi-imputation')); ?>" class="flex flex-col gap-2 md:flex-row md:items-center">
            <select name="tri_date" onchange="this.form.submit()"
                class="border border-gray-200 rounded-xl px-4 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-300 text-gray-600">
                <option value="recent" <?php echo e($suiviSort === 'recent' ? 'selected' : ''); ?>>Date récente</option>
                <option value="ancien" <?php echo e($suiviSort === 'ancien' ? 'selected' : ''); ?>>Date ancienne</option>
            </select>
            <div class="relative min-w-[260px]">
                <input type="text" name="q" value="<?php echo e($suiviSearch); ?>"
            ...
            <?php if($suiviSearch !== ''): ?>
            <a href="<?php echo e(route('courrier.suivi-imputation')); ?>"
                class="px-4 py-2 bg-gray-100 text-gray-600 rounded-xl text-sm font-semibold hover:bg-gray-200 transition flex items-center gap-1.5">
                <i class="fas fa-times text-xs"></i> Réinitialiser
            </a>
            <?php endif; ?>
    ...
    <table class="w-full min-w-[1100px] text-sm">
        ...
        <tbody class="divide-y divide-gray-100">
            <?php $__empty_1 = true; $__currentLoopData = $suiviImputations; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $si): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            <tr class="hover:bg-gray-50 transition">
                <td class="px-6 py-4 font-bold text-gray-800"><?php echo e($si['num']); ?></td>
                <td class="px-6 py-4 text-gray-700 max-w-xs"><?php echo e($si['objet']); ?></td>
                <td class="px-6 py-4 text-gray-700">
                    ...
                    ?>
                    <span class="inline-flex items-center mt-1 px-2.5 py-1 rounded-full text-[11px] font-semibold <?php echo e($suiviBadge); ?>">
                        <?php echo e($si['suivi_statut'] ?? 'En cours de traitement'); ?>

                    </span>
                </td>
                <td class="px-6 py-4 text-gray-700"><?php echo e($si['instruction'] ?? '—'); ?></td>
                <td class="px-6 py-4 text-gray-600 whitespace-nowrap">
                    <div class="flex items-center gap-2">
                        <span><?php echo e($si['delai_traitement'] ?? '—'); ?></span>
                        <?php if($si['delai_alert_one_day'] ?? false): ?>
                ...
                <td class="px-6 py-4 text-gray-600 whitespace-nowrap"><?php echo e($si['date_concernee']); ?></td>
                <td class="px-6 py-4 text-gray-600"><?php echo e($si['impute_a']); ?></td>
                <td class="px-6 py-4 text-gray-400 text-xs">
                    <?php if($si['fichier_reponse']): ?>
                        <a href="<?php echo e($si['fichier_reponse']); ?>" target="_blank"
                ...
                <td class="px-6 py-4">
                    <?php if($si['can_validate'] ?? false): ?>
                    <div class="flex items-center gap-2">
                        <button
                            type="button"
                            onclick="openDecisionModal('ok', '<?php echo e(route('courrier.ok-traitement', $si['id'])); ?>', '<?php echo e(addslashes($si['num'])); ?>', '<?php echo e(addslashes($si['objet'])); ?>')"
                            class="px-3 py-1.5 rounded-lg bg-green-600 text-white text-xs font-semibold hover:bg-green-700">
                            <i class="fas fa-check mr-1"></i> OK
                        </button>
                        <button
                            type="button"
                            onclick="openDecisionModal('valider', '<?php echo e(route('courrier.valider-traitement', $si['id'])); ?>', '<?php echo e(addslashes($si['num'])); ?>', '<?php echo e(addslashes($si['objet'])); ?>')"
                        ...
                        <button
                            type="button"
                            onclick="openDecisionModal('rejeter', '<?php echo e(route('courrier.rejeter-traitement', $si['id'])); ?>', '<?php echo e(addslashes($si['num'])); ?>', '<?php echo e(addslashes($si['objet'])); ?>')"
...
<div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
    ...
    <div class="px-6 py-5 border-b border-gray-100 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
            ...
            <p class="text-xs text-gray-400 mt-1"><?php echo e($courrierTraites->count()); ?> courrier(s) trouvé(s)</p>
        </div>
        <form method="GET" action="<?php echo e(route('courrier.traite')); ?>" class="flex flex-col gap-2 md:flex-row md:items-center">
            <select name="tri_date" onchange="this.form.submit()"
                class="border border-gray-200 rounded-xl px-4 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-300 text-gray-600">
                <option value="recent" <?php echo e($traiteSort === 'recent' ? 'selected' : ''); ?>>Date récente</option>
                <option value="ancien" <?php echo e($traiteSort === 'ancien' ? 'selected' : ''); ?>>Date ancienne</option>
            </select>
            <select name="statut_filtre" onchange="this.form.submit()"
                class="border border-gray-200 rounded-xl px-4 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-300 text-gray-600">
                <option value="" <?php echo e($traiteStatusFilter === '' ? 'selected' : ''); ?>>Tous les statuts</option>
                <option value="En attente validation" <?php echo e($traiteStatusFilter === 'En attente validation' ? 'selected' : ''); ?>>En attente validation</option>
                <option value="Validé" <?php echo e($traiteStatusFilter === 'Validé' ? 'selected' : ''); ?>>Validé</option>
                <option value="Rejeté" <?php echo e($traiteStatusFilter === 'Rejeté' ? 'selected' : ''); ?>>Rejeté</option>
            </select>
            <div class="relative min-w-[260px]">
                <input type="text" name="q" value="<?php echo e($traiteSearch); ?>"
                    placeholder="Rechercher par numéro ou objet…"
                    class="w-full border border-gray-200 rounded-xl pl-9 pr-4 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-300 placeholder-gray-400">
                <i class="fas fa-search absolute left-3 top-1/2 -translate-y-1/2 text-gray-300 text-xs"></i>
            </div>
            ...
            <?php if($traiteStatusFilter !== '' || $traiteSearch !== ''): ?>
            <a href="<?php echo e(route('courrier.traite')); ?>"
                class="px-4 py-2 bg-gray-100 text-gray-600 rounded-xl text-sm font-semibold hover:bg-gray-200 transition flex items-center gap-1.5">
                <i class="fas fa-times text-xs"></i> Réinitialiser
            </a>
            <?php endif; ?>
    ...
    <table class="w-full min-w-[1100px] text-sm">
        ...
        <tbody class="divide-y divide-gray-100">
            <?php $__empty_1 = true; $__currentLoopData = $courrierTraites; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $ct): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            <tr class="hover:bg-gray-50 transition">
                <td class="px-6 py-5 font-bold text-gray-800 whitespace-nowrap"><?php echo e($ct['num']); ?></td>
                <td class="px-6 py-5 text-gray-700 max-w-xs"><?php echo e($ct['objet']); ?></td>
                <td class="px-6 py-5 text-gray-600"><?php echo e($ct['traitement']); ?></td>
                <td class="px-6 py-5 text-gray-600 whitespace-nowrap"><?php echo e($ct['date_concernee']); ?></td>
                <td class="px-6 py-5">
                    <?php
                    ...
                    ?>
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold <?php echo e($ctClass); ?>">
                        <?php echo e($ct['statut']); ?>

                    </span>
                </td>
                <td class="px-6 py-5">
                    <?php if($ct['fichier_reponse']): ?>
                    <a href="<?php echo e($ct['fichier_reponse']); ?>" target="_blank"
                ...
                <td class="px-6 py-5">
                    <button onclick="openPdfModal('<?php echo e($ct['fichier_courrier'] ?? ''); ?>', '<?php echo e($ct['num']); ?>')"
...
<?php elseif($subtab === 'archives'): ?>

<div class="flex flex-wrap items-center justify-between gap-3 mb-5">
    <div>
        <h2 class="text-2xl font-bold text-gray-900">Courriers Archivés</h2>
        <?php if(isset($archivalDays) && $archivalDays > 0): ?>
        <p class="text-xs text-gray-400 mt-0.5">
            <i class="fas fa-clock mr-1"></i>
            Courriers enregistrés il y a plus de <strong><?php echo e($archivalDays); ?> jours</strong>
        </p>
        <?php endif; ?>
    </div>
</div>
...
<form method="GET" action="<?php echo e(route('courrier.archives')); ?>" class="mb-5">
    <div class="flex flex-wrap gap-3 items-center">
        <div class="flex gap-2">
            <?php $__currentLoopData = ['tous' => 'Tous', 'arrive' => 'Arrivés', 'depart' => 'Départs']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $val => $lbl): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <button type="submit" name="filtre" value="<?php echo e($val); ?>"
                class="<?php echo e(($filtre ?? 'tous') === $val ? 'bg-stone-600 text-white shadow-sm' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50'); ?> px-5 py-2 rounded-xl text-sm font-semibold transition">
                <?php echo e($lbl); ?>
        ...
        <select name="tri" onchange="this.form.submit()"
            ...
            <option value="recent" <?php echo e(($sort ?? 'recent') === 'recent' ? 'selected' : ''); ?>>Plus récent d'abord</option>
            <option value="ancien" <?php echo e(($sort ?? 'recent') === 'ancien' ? 'selected' : ''); ?>>Plus ancien d'abord</option>
        </select>
        <input type="hidden" name="filtre" value="<?php echo e($filtre ?? 'tous'); ?>">
        ...
        <?php if(($filtre ?? 'tous') !== 'tous' || ($search ?? '') !== ''): ?>
        <a href="<?php echo e(route('courrier.archives')); ?>"
            class="px-4 py-2 bg-gray-100 text-gray-600 rounded-xl text-sm font-semibold hover:bg-gray-200 transition flex items-center gap-1.5">
            <i class="fas fa-times text-xs"></i> Réinitialiser
        </a>
        <?php endif; ?>
...
<p class="text-xs text-gray-400 mb-3"><?php echo e(count($courriers)); ?> courrier(s) archivé(s) trouvé(s)</p>
    ...
    <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-gray-100">
            ...
            <tr class="hover:bg-stone-50/50 transition opacity-80">
                ...
                <td class="px-6 py-4 text-gray-600"><?php echo e($c['objet']); ?></td>
                <td class="px-6 py-4">
                    <span class="inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-1 rounded-full
                        <?php echo e($c['type'] === 'Arrivé' ? 'bg-indigo-50 text-indigo-600' : 'bg-orange-50 text-orange-600'); ?>">
                        <i class="fas <?php echo e($c['type'] === 'Arrivé' ? 'fa-arrow-down' : 'fa-arrow-up'); ?> text-[10px]"></i>
                        <?php echo e($c['type']); ?>
                ...
                <td class="px-6 py-4">
                    <?php $priClass = match($c['priorite']) { 'Urgent' => 'text-orange-500', 'Très urgent' => 'text-red-500', default => 'text-gray-500' }; ?>
                    <span class="<?php echo e($priClass); ?> text-sm font-semibold"><?php echo e($c['priorite']); ?></span>
                </td>
                <td class="px-6 py-4">
                    ...
                    <span class="<?php echo e($badge); ?> px-3 py-1 rounded-full text-xs font-semibold"><?php echo e($c['statut']); ?></span>
                </td>
                <td class="px-6 py-4 text-gray-500 text-xs whitespace-nowrap"><?php echo e($c['date']); ?></td>
                <td class="px-6 py-4 text-gray-500"><?php echo e($c['agent']); ?></td>
                <td class="px-6 py-4 text-right">
...
<div id="modal-edit" class="fixed inset-0 z-50 hidden flex items-center justify-center p-4">
    ...
    <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl flex flex-col" style="max-height:90vh">
        ...
        <form method="POST" action="<?php echo e(route('courrier.store')); ?>" enctype="multipart/form-data" class="flex flex-col flex-1 min-h-0">
...
<div id="modal-imputer" class="fixed inset-0 z-50 hidden flex items-center justify-center p-4">
    ...
    <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl flex flex-col" style="max-height:92vh">
        ...
        <div class="overflow-y-auto flex-1 px-6 py-5">
            <form id="form-imputer" method="POST" action="<?php echo e(route('courrier.imputer')); ?>" class="space-y-5">
            <?php echo csrf_field(); ?>
            ...
            <div>
                ...
                <div class="flex flex-col md:flex-row gap-2 items-stretch mb-3">
                    <select id="select-entite" class="w-full md:flex-1 min-w-0 border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400 bg-white">
                        ...
                        <?php $__empty_1 = true; $__currentLoopData = ($imputationEntities ?? collect()); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $ent): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <option value="<?php echo e($ent->code); ?>" data-label="<?php echo e($ent->name); ?>"><?php echo e($ent->name); ?> (<?php echo e($ent->code); ?>)</option>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                    ...
                    <select id="select-instruction" class="w-full md:flex-1 min-w-0 border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400 bg-white">
                        ...
                        <?php $__currentLoopData = \App\Models\Instruction::where('actif', true)->latest()->get(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $instr): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <option value="<?php echo e($instr->id); ?>" data-label="<?php echo e($instr->nom); ?>"><?php echo e($instr->nom); ?></option>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </select>
...
var __ooViewer = <?php echo json_encode($ooViewer ?? 'native', 15, 512) ?>;
var __ooUrl    = <?php echo json_encode($ooUrl    ?? '', 15, 512) ?>;
var __visualiserUrl = <?php echo json_encode(route('courrier.visualiser'), 15, 512) ?>;
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\wamp64\www\e-administration_laravel\resources\views/courrier/index.blade.php ENDPATH**/ ?>