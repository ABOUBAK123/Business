...
# ============================================================================
# Script de déploiement Laravel - Ubuntu 22.04 + Apache (compatible Webmin)
# Utilisation :
...
#   1) Modifier les variables dans la section "CONFIGURATION"
...
# Mettre à 1 pour demander automatiquement un certificat Let's Encrypt
ENABLE_SSL="1"
LETSENCRYPT_EMAIL="jjkam555@yahoo.ci"  # <-- ton email pour les alertes SSL
# -----------------------------------------------------------------------------

if [[ "$EUID" -ne 0 ]]; then
  echo "Lancer en root : sudo bash $0"
  exit 1
fi

export DEBIAN_FRONTEND=noninteractive

echo "[1/11] Installation des paquets système..."
apt-get update -y
...
echo "[1b/11] Configuration des limites d'upload PHP..."
...
if ! command -v composer >/dev/null 2>&1; then
  echo "[2/11] Installation de Composer..."
...
if [[ "$USE_GIT" == "1" ]]; then
  if [[ "$REPO_URL" == "https://github.com/your-org/your-repo.git" ]]; then
    echo "REPO_URL est encore la valeur par défaut. Définissez REPO_URL ou mettez USE_GIT=0."
    exit 1
  fi

  if [[ ! -d "$APP_DIR/.git" ]]; then
    echo "[3/11] Clonage du dépôt..."
    mkdir -p "$(dirname "$APP_DIR")"
    git clone --branch "$REPO_BRANCH" "$REPO_URL" "$APP_DIR"
  else
    echo "[3/11] Récupération du dernier code..."
    git -C "$APP_DIR" fetch --all --prune
    git -C "$APP_DIR" checkout "$REPO_BRANCH"
    git -C "$APP_DIR" pull --ff-only origin "$REPO_BRANCH"
  fi
else
  echo "[3/11] Étape Git ignorée (USE_GIT=0)."
  if [[ ! -f "$APP_DIR/artisan" ]]; then
    echo "Aucun projet Laravel trouvé dans $APP_DIR (artisan manquant)."
    exit 1
  fi
fi
...
echo "[4/11] Installation des dépendances PHP..."
composer install --no-dev --optimize-autoloader --no-interaction

echo "[4b/11] Vérification des assets PDF.js..."
...
if [[ ! -f .env ]]; then
  echo "[5/11] Création du fichier .env depuis .env.example..."
  cp .env.example .env
fi

echo "[5/11] Mise à jour des valeurs du fichier .env..."
...
# Synchroniser l'URL publique OnlyOffice avec APP_URL
...
echo "app_public_url mis à jour\n";
'

echo "[7/11] Correction des permissions..."
chown -R www-data:www-data "$APP_DIR"
find "$APP_DIR" -type f -exec chmod 644 {} \;
find "$APP_DIR" -type d -exec chmod 755 {} \;
chmod -R ug+rwx "$APP_DIR/storage" "$APP_DIR/bootstrap/cache"

echo "[8/11] Configuration du vhost Apache..."
...
echo "[9/11] Configuration du cron planificateur..."
...
if [[ "$ENABLE_SSL" == "1" ]]; then
  echo "[10/11] Activation du certificat SSL Let's Encrypt..."
  apt-get install -y certbot python3-certbot-apache
  certbot --apache --non-interactive --agree-tos -m "$LETSENCRYPT_EMAIL" -d "$DOMAIN" -d "$DOMAIN_WWW" --redirect
else
  echo "[10/11] Étape SSL ignorée (ENABLE_SSL=0)."
fi

echo "[11/11] Vérifications post-installation..."
...
' >/tmp/eparapheur_db_check.log 2>&1; then
  DB_CHECK="OK"
  echo "[CHECK] Connexion base de données : OK"
else
  echo "[CHECK] Connexion base de données : ÉCHEC"
...
if [[ "$APP_HTTP_CODE" =~ ^(200|301|302)$ ]]; then
  echo "[CHECK] URL publique https://${DOMAIN} : OK (HTTP ${APP_HTTP_CODE})"
else
  echo "[CHECK] URL publique https://${DOMAIN} : ATTENTION (HTTP ${APP_HTTP_CODE})"
fi
...
if [[ -n "$OO_URL" ]]; then
  ...
  if [[ "$OO_HEALTH_CODE" == "200" ]]; then
    ...
    echo "[CHECK] OnlyOffice healthcheck ${OO_URL%/}/healthcheck : ATTENTION (HTTP ${OO_HEALTH_CODE})"
  fi
else
  echo "[CHECK] OnlyOffice healthcheck : IGNORÉ (onlyoffice_server_url non défini)"
fi

echo "Terminé. Déploiement effectué avec succès."
echo "Ouvrir : https://${DOMAIN}"
